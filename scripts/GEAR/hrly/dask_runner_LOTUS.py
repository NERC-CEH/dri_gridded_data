...
class DaskRunner(BundleBasedDirectRunner):
  ...
  def run_pipeline(self, pipeline, options):
    ...
    LOTUS = SLURMCluster(
      cores           = dask_options["worker_cores"],
      memory          = str(int(dask_options["worker_memory"]))+"GB",
      queue           = 'short-serial-4hr',
      account         = 'short4hr',
      walltime        = '02:00:00',
      local_directory = '/work/scratch-nopw2/mattjbr'
    )
    
    # set the minimum/maximum number of workers - I imagine this will need to match
    # the number of workers we configure Beam with
    LOTUS.scale(dask_options["workers"])
    # get dashboard link to monitor progress
    print('Dask Dashboard link: ' + LOTUS.dashboard_link + '\n\n')

    client = ddist.Client(LOTUS)
