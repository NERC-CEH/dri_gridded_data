...
class CoordVarOverwrite(beam.PTransform):
    ...
    def _coordvar_overwrite(
        item: Indexed[T], config, chunkdims, concdims, owfile
    ) -> Indexed[T]:
        import numpy as np
        import xarray as xr
        ...
        if config.overwrites == "on":
            ...
            if not config.var_overwrites:
                ...
                for key in ds.variables.keys():
                    ...
                    if ndims != nchunkdims:  # rule out the main dataset variables
                        ...
                        concdimmatches = [
                            concdim in ds[key].dims for concdim in concdims
                        ]
                        ...
                        if key not in chunkdims and not matches_bool:
                            logging.info(
                                "Adding " + key + " to list of vars to replace"
                            )
            ...
            for vari in vars_to_replace:
                logging.info(
                    "Replacing values of " + vari + " with those from " + owfile
                )
                try:
...
def convert_wrf(config: Config):
    ...
    if config.num_workers > 1:
        beam_options = PipelineOptions(
            direct_num_workers=config.num_workers,
